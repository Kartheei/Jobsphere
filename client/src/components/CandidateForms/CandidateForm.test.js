...
describe("CandidateForm Component", () => {
  ...
  test("renders the component correctly", () => {
    ...
    expect(
      screen.getByAltText(
        "Three people standing on a table and searching htmlFor something through telescope in three directions and a masked figure siting under the table."
      )
    ).toBeInTheDocument();
    expect(
      screen.getByAltText(
        "Logo htmlFor Job Sphere: A circular emblem with the words 'Job Sphere' in bold, modern font, surrounded by interconnected gears symbolizing career opportunities."
      )
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("Already have an account? signin")
    ).toBeInTheDocument();
  });
